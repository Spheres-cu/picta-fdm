...
def main():
    ...
    print(f'Building picta-dl v{version} for {OS_NAME} {platform.machine()} with options {opts}')
    print('Remember to update the version using  "devscripts/update-version.py"')
    if not os.path.isfile('picta_dl/extractor/lazy_extractors.py'):
    ...
    opts = [
        ...
        '--noconfirm',
        '--additional-hooks-dir=picta_dl/__pyinstaller',
        *opts,
    ...
    SetVersion(exe, VSVersionInfo(
        ...
        kids=[
            StringFileInfo([StringTable('040904B0', [
                StringStruct('Comments', f'picta-dl{suffix} Command Line Interface'),
                StringStruct('CompanyName', 'https://github.com/picta-dl'),
                StringStruct('FileDescription', 'picta-dl%s' % (MACHINE and f' ({MACHINE})')),
                StringStruct('FileVersion', version),
                StringStruct('InternalName', f'picta-dl{suffix}'),
                StringStruct('OriginalFilename', f'picta-dl{suffix}.exe'),
                StringStruct('ProductName', f'picta-dl{suffix}'),
                StringStruct(
